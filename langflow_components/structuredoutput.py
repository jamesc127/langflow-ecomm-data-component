...
class StructuredOutputComponent(Component):
    display_name = "List Structured Output"
    description = (
        "Transforms LLM responses into **structured data formats**. Ideal for extracting specific information "
        "or creating consistent outputs. Returns a **List[Data]** object."
    )
    ...
    def build_structured_output(self) -> List[Data]:
        ...
        def construct_data_item(item) -> Data:
            """Construct a Data object with item assigned to both data and metadata."""
            return Data(
                data=item,
                metadata=item
            )
        ...
        if isinstance(output, BaseModel):
            ...
            if self.multiple:
                return [construct_data_item(item) for item in output_dict.get('objects', [])]
            else:
                return [construct_data_item(output_dict)]
        else:
